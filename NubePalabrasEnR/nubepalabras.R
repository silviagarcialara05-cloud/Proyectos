#Nombre: Sheila san Jose Baraja, Silvia Garcia Lara, Marina Palomino Duque

#PRIMERA PARTE

#1.- Instalar los paquetes tm, wordcloud y wordcloud2 en R. 縋ara que sirven?
...
#Para saber para qu茅 sirve cada uno de los paquetes instalados anteriormente, hacemos uso de internet 
#y de la instrucci贸n help() que nos proporciona R:
help(tm)
help(wordcloud)
help(wordcloud2)
...
myCorpus = VCorpus(VectorSource(plain.text)) #Creacion del objeto myCorpus de tipo VCorpus previamente habiendo aplicado VectorSource al 
#texto leido.
...
myCorpus = tm_map(myCorpus, removeWords, stopwords("spanish")) #Eliminar palabras que usamos con mucha frecuencia pero que no tienen un 
#significado concreto en espanol.
myCorpus <- tm_map(myCorpus, PlainTextDocument) #Elimina el formato. 
myDTM = TermDocumentMatrix(myCorpus, control = list(minWordLength = 1)) #Convierte el corpus en una tabla de frecuencia considerando 
#palabras con una longitud minima de 1 caracter
...
wordcloud(names(v),v, min.freq = 2, colors=brewer.pal(6,"Dark2"),  random.order=FALSE) #Creacion de una nube de palabras seleccionando 
#como palabras los nombres y las frecuencias que contiene v , eligiendo aquellos que tengan una frecuencia igual o mayor que 2 y usando  
#como colores de las palabras la paleta brewer(6, Dark2). Por ultimo se ordenan las palabras dependiendo de la frecuencia de cada una.

#3.- Considerar el texto que se encuentra en el archivo Texto.txt que corresponde al artculo publicado 
#en el periodico eldiario.es titulado El gigante estadounidense del carbon financiaba a decenas de grupos 
#que niegan el cambio climatico.

#A partir de Texto.txt mostrar un objeto que contenga las palabras que aparezcan 10 o mas veces en el texto 
#(que no sean conectores ni articulos, etc propios de la gramatica) junto con la frecuencia de cada una de ellas. 
...
#Realizamos un gr谩fico de barras con estas palabras
barplot(height = mywords2$freq, names = mywords2$words, 
        col=c('dodgerblue4','dodgerblue3','dodgerblue2','dodgerblue1','dodgerblue'))

#Realizamos la nube de palabras para el conjunto de datos
wordcloud(mywords2$words,mywords2$freq,min.freq=1,random.order=F,
          colors =brewer.pal(8, "Accent"))


#Realizar varias nubes de palabras que consideres de inter茅s. En todos los casos muestra la tabla de frecuencias 
#correspondiente.
...
#4.- Realizar una nube de palabras para mostrar las 10 palabras mas frecuentes en cada una de las hojas de 
#ejercicios sobre R de este curso. Luego hacer una unica nube de palabras con las 10 palabras mas frecuentes 
#en todas esas hojas de ejercicios.

#instalamos el paquete pdftools para poder leer los pdf
install.packages('pdftools')

#Realizamos la nube de palabras mostrando las 10 palabras mas frecuentes en cada una de las hojas
#de ejercicios sobre R de este curso:
...
#5.- Algunas ordenes utiles para trabajar con caracteres en R son: 
#nchar , sub, substr, strsplit, regexpr, gregexpr, grep, sub, gsub.

#Explicar para que se utilizan y poner un ejemplo cada una de ellas.

#Para saber para qu茅 se utilizan las 贸rdenes del enunciado, hacemos uso de internet y de la instrucci贸n help() que 
...
#-sub: La funcion sub se usa muy a menudo para limpieza de datos. Una llamada a sub 
#tiene la forma sub(x, y, <vector de caracteres>) donde el primer argumento, x, es una 
#expresion regular; la funcion gsub modifica las ocurrencias de esta expresion regular 
#por el segundo argumento, y, en este caso. El tercer argumento es un vector que contiene 
#cadenas de texto en las que se realiza la sustitucion. En caso de que algun elemento del 
#vector de caracteres contenga mas de una ocurrencia del argumento x, solo se modifica la 
...
#-strsplit: separa los elementos de un vector de tipo caracter en subcadenas de acuerdo 
#con el delimitador proporcionado.
...
strsplit(x, "e")
#lo divide por la posici贸n en la que se encuentran las letras e

#-regexpr: se usa para identificar donde se encuentra un patron dentro de un vector de 
#caracteres en el cual, cada elemento se busca por separado. Devuelve la posicion de la 
#primera coincidencia con el atributo match.length. Si no encuentra coincidencias devuelve
#-1. Lo devuelve en forma de vector.

#Ejemplo:
regexpr("t", "orquesta")

#-gregexpr: Similar a la funcion regexpr Se usa para identificar donde se encuentra un 
#patron dentro de un vector de caracteres donde cada elemento se busca por separado. 
#Devuelve la informacion en forma de lista. Si no encuentra el patron, devuelve un -1.
...
#donde el primer argumento, x, es una expresion regular; la funcion gsub modifica las ocurrencias de esta 
#expresion regular por el segundo argumento, y, en este caso. El tercer argumento es un vector que contiene 
#cadenas de texto en las que se realiza la sustitucion.

#Ejemplo:
gsub("h", "H", c("hola", "buho"))


#6.- Cada linea del fichero SMSSpam.txt se refiere a un mensaje de texto SMS. 
#Cada linea de dicho fichero esta compuesta de dos columnas:
...
#7.- 7.- Anadir una columna mas al data frame mensajes que contenga para cada uno su longitud. 
#縎on mas cortos los mensajes spam o los legitimos?
...
#8.- Para intentar discernir entre los SMS que son spam y los legitimos, realizar nubes de palabras 
#para los mensajes spam y para los legitimos. 縃ay grandes diferencias?.
...
#A simple vista podemos ver que en los mensajes Spam, a diferencia de  las palabras en los mensajes legitimos, 
#las  mas repetidas son algunas como "call" , "free" o "service", que nos hacen ver que son mensajes con un alto 
#contenido de publicidad.


#9.- Realiza los resumenes y graficos que consideres interesantes para poder responder a la pregunta 縣ay diferencia 
#entre los mensajes spam o no?

#Para poder comparar los dos conjuntos de datos vamos a realizar diferentes graficas 
#apropiadas para ello.

#Con respecto al numero de palabras que se repiten podemos ver lo siguiente:
  
#Al tener las frecuencias con las que se repiten las palabras en ambos tipos de mensaje, la mejor manera de compararlos 
#es observar las palabras mas repetidas de ambos conjuntos de datos para ello creamos data frames con las 6 palabras mas 
#repetidas de cada uno
...
#Se observa que la palabra mas repetida en los mensajes Spam tiene una frecuencia muy cercana a 500, 
#mientras que la segunda tiene una frecuencia alrededor de 300. La frecuencia del resto de palabras 
#sigue siendo menor que estas dos, pero ya no se observa una diferencia tan grande en la frecuencia 
#como con estas dos primeras palabras.
...
#Al comparar ambos graficos de barras vemos que los mensajes Spam tienen un numero considerablemente 
#mas pequeno de palabras repetidas que los mensajes Ham.
